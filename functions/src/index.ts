...
const ACHIEVEMENT_DEFS: {
  ...
  // COMBAT — Quests
  { id: 'first_mission', conditionType: 'QUESTS_COMPLETED', conditionTarget: 1, xpReward: 50 },
  { id: 'mission_5', conditionType: 'QUESTS_COMPLETED', conditionTarget: 5, xpReward: 150 },
  { id: 'mission_20', conditionType: 'QUESTS_COMPLETED', conditionTarget: 20, xpReward: 400, fluxReward: 200 },
  { id: 'mission_50', conditionType: 'QUESTS_COMPLETED', conditionTarget: 50, xpReward: 1000, fluxReward: 500 },
  // COMBAT — Boss Kills
  ...
  // SOCIAL — Tutoring
  { id: 'tutor_1', conditionType: 'TUTORING_SESSIONS', conditionTarget: 1, xpReward: 75 },
  { id: 'tutor_10', conditionType: 'TUTORING_SESSIONS', conditionTarget: 10, xpReward: 300, fluxReward: 200 },
  { id: 'tutor_25', conditionType: 'TUTORING_SESSIONS', conditionTarget: 25, xpReward: 750, fluxReward: 500 },
...
function checkAndUnlockAchievements(
  ...
  const projectedQuests = (gam.completedQuests || []).length;
  const projectedStreak = gam.engagementStreak || 0;
  const projectedLoginStreak = gam.loginStreak || 0;
  const projectedTutoring = gam.tutoringSessionsCompleted || 0;
  ...
  const conditionValues: Record<string, number> = {
    ...
    QUESTS_COMPLETED: projectedQuests,
    STREAK_WEEKS: projectedStreak,
    LOGIN_STREAK: projectedLoginStreak,
    TUTORING_SESSIONS: projectedTutoring,
 ...
 */
async function writeAchievementNotifications(
...
export const awardXP = onCall({ minInstances: 1 }, async (request) => {
  ...
  return db.runTransaction(async (transaction) => {
    ...
    const { rewardUpdates: xpAchievementUpdates, newUnlocks: xpNewUnlocks } =
      checkAndUnlockAchievements(data, updates, true);
    Object.assign(updates, xpAchievementUpdates);

    transaction.update(userRef, updates);
    return { newXP, newLevel, leveledUp, newUnlocks: xpNewUnlocks };
  });
});
...
export const craftItem = onCall(async (request) => {
  ...
  return db.runTransaction(async (transaction) => {
    ...
    // Increment itemsCrafted counter and check achievements
    updates["gamification.itemsCrafted"] = (data.gamification?.itemsCrafted || 0) + 1;
    const { rewardUpdates: craftAchievementUpdates, newUnlocks: craftNewUnlocks } =
      checkAndUnlockAchievements(data, updates);
    Object.assign(updates, craftAchievementUpdates);

    transaction.update(userRef, updates);
    return { item, newCurrency: currentCurrency - actualCost, usedRerollToken: useRerollToken, newUnlocks: craftNewUnlocks };
  });
});
