/**
 * One-shot: invoke a callable CF AS A SPECIFIC USER (by uid), not admin.
 * Usage: npx tsx invoke-as-user.ts <uid> <fnName> '<jsonPayload>'
 */
...
async function main() {
  ...
  const fnResp = await fetch(url, {
    method: "POST",
    headers: { "Content-Type": "application/json", "X-Firebase-Protocol": "true",
