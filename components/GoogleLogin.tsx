...
const GoogleLogin: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[var(--surface-raised)] p-6 relative overflow-hidden font-sans">
      ...
      <div className={`max-w-md w-full bg-[var(--surface-glass)] border border-[var(--border)] rounded-2xl shadow-2xl overflow-hidden z-10 ${prefersReducedMotion ? '' : 'animate-in fade-in zoom-in-95 duration-300'}`}>
        <div className="p-10 text-center">
          ...
          <button
            ...
            disabled={loading}
            className="w-full flex items-center justify-center gap-3 bg-[var(--surface-raised)] border border-[var(--border-strong)] hover:bg-[var(--surface-sunken)] text-[var(--text-primary)] font-medium py-3 px-4 rounded-xl transition-all shadow-sm hover:shadow-md group disabled:opacity-70 disabled:cursor-not-allowed mb-6"
          >
            {loading ? (
                <div className="animate-spin rounded-full h-5 w-5 border-t-2 border-[var(--accent)]"></div>
            ) : (
                <>
