  ...
  return (
    <div className="space-y-6" style={{ animation: 'tabEnter 0.3s ease-out both' }}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <MetricCard label="Class XP" value={classXp.toLocaleString()} sub={`${totalXp.toLocaleString()} total across all classes`} color="text-amber-600 dark:text-amber-400" icon={<Zap className="w-3.5 h-3.5" />} />
        <MetricCard label="Gear Score" value={gearScore} sub={`${Object.values(equipped).filter(Boolean).length}/8 slots filled`} color="text-purple-600 dark:text-purple-400" icon={<Shield className="w-3.5 h-3.5" />} />
        <MetricCard label="Resources Accessed" value={classSubmissions.length} sub={`${statusBreakdown.SUCCESS} scored well`} color="text-emerald-700 dark:text-emerald-400" icon={<Target className="w-3.5 h-3.5" />} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-[var(--surface-glass)] border border-[var(--border-strong)] rounded-2xl p-5 space-y-4">
          <h3 className="text-xs font-bold text-[var(--text-tertiary)] uppercase tracking-widest flex items-center gap-2">
            <Swords className="w-4 h-4 text-red-600 dark:text-red-400" /> Combat Analysis
          </h3>
          <p className="text-[11px] text-[var(--text-muted)] -mt-2">How your character stats translate to boss fight performance</p>

          <div className="space-y-3">
            <StatBar label="Tech" value={playerStats.tech} max={100} color="text-blue-600 dark:text-blue-400" icon={<Zap className="w-3 h-3" />} description="Attack Power — Increases base damage dealt to bosses in quiz events." />
            <StatBar label="Focus" value={playerStats.focus} max={100} color="text-green-600 dark:text-green-400" icon={<Crosshair className="w-3 h-3" />} description={`Critical Strikes — ${(combat.critChance * 100).toFixed(0)}% crit chance, ${combat.critMultiplier.toFixed(2)}x crit multiplier.`} />
            <StatBar label="Analysis" value={playerStats.analysis} max={100} color="text-yellow-600 dark:text-yellow-400" icon={<Shield className="w-3 h-3" />} description={`Armor — Reduces incoming boss damage by ${combat.armorPercent.toFixed(0)}%.`} />
            <StatBar label="Charisma" value={playerStats.charisma} max={100} color="text-purple-600 dark:text-purple-400" icon={<Heart className="w-3 h-3" />} description={`Health Pool — Gives you ${combat.maxHp} HP in boss fights (base 100 + 5 per charisma above 10).`} />
          </div>
            ...
            <div className="pt-3 border-t border-[var(--border)] space-y-2">
              ...
              <div className="grid grid-cols-2 gap-1.5 text-[11.5px]">
                ...
                {Object.keys(runewordStats).length > 0 && (
                  <div className="flex justify-between text-orange-600 dark:text-orange-400/70"><span>Runewords</span><span>+{Object.values(runewordStats).reduce((a, b) => a + b, 0)}</span></div>
                )}
                {Object.keys(setBonusStats).length > 0 && (
                  <div className="flex justify-between text-amber-600 dark:text-amber-400/70"><span>Set Bonus</span><span>+{Object.values(setBonusStats).reduce((a, b) => a + b, 0)}</span></div>
                )}
              </div>
            </div>
          )}
        ...
        <div className="bg-[var(--surface-glass)] border border-[var(--border-strong)] rounded-2xl p-5 space-y-4">
          <h3 className="text-xs font-bold text-[var(--text-tertiary)] uppercase tracking-widest flex items-center gap-2">
            <Zap className="w-4 h-4 text-amber-600 dark:text-amber-400" /> XP Sources
          </h3>
          <p className="text-[11px] text-[var(--text-muted)] -mt-2">Where your {classXp.toLocaleString()} XP in {activeClass} came from</p>

          <div className="space-y-2">
            {[
              { label: 'Resource Engagement', xp: xpBreakdown.engagementXp, color: 'bg-purple-500', textColor: 'text-purple-600 dark:text-purple-400', desc: '~10 XP per minute of active engagement' },
              { label: 'Boss Encounters', xp: xpBreakdown.bossXp, color: 'bg-red-500', textColor: 'text-red-600 dark:text-red-400', desc: 'XP from boss quiz damage' },
              { label: 'Other (Badges, Daily, Behavior)', xp: xpBreakdown.otherXp, color: 'bg-amber-500', textColor: 'text-amber-600 dark:text-amber-400', desc: 'Login rewards, behavior awards, badge XP' },
          ...
          {(streakMultiplier > 1) && (
            <div className="bg-orange-500/10 border border-orange-500/20 rounded-xl p-3 mt-2">
              <div className="flex items-center gap-2 text-xs">
                ...
                <span className="text-orange-600 dark:text-orange-400 font-bold">Streak Bonus Active</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-[var(--surface-glass)] border border-[var(--border-strong)] rounded-2xl p-5 space-y-4">
          <h3 className="text-xs font-bold text-[var(--text-tertiary)] uppercase tracking-widest flex items-center gap-2">
            <BarChart3 className="w-4 h-4 text-cyan-600 dark:text-cyan-400" /> 7-Day Activity
          </h3>
      ...
      <div className="bg-[var(--surface-glass)] border border-[var(--border-strong)] rounded-2xl p-5 space-y-4">
        <h3 className="text-xs font-bold text-[var(--text-tertiary)] uppercase tracking-widest flex items-center gap-2">
          <Swords className="w-4 h-4 text-red-600 dark:text-red-400" /> Boss Fight Readiness
        </h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="bg-[var(--panel-bg)] rounded-xl p-4 border border-red-500/10 space-y-2">
            ...
            <div className="text-sm text-[var(--text-secondary)]">
              Your <span className="text-blue-600 dark:text-blue-400 font-bold">{playerStats.tech} Tech</span> powers your base attack.
              {playerStats.tech > 20 && <span className="text-emerald-700 dark:text-emerald-400"> Strong offensive capability.</span>}
              {playerStats.tech <= 20 && playerStats.tech > 10 && <span className="text-yellow-600 dark:text-yellow-400"> Moderate damage output.</span>}
              {playerStats.tech <= 10 && <span className="text-red-600 dark:text-red-400"> Low damage — equip Tech gear to improve.</span>}
            </div>
            <div className="text-[11.5px] text-[var(--text-muted)]">
              Crit chance: <span className="text-green-600 dark:text-green-400 font-bold">{(combat.critChance * 100).toFixed(0)}%</span> (from {playerStats.focus} Focus)
            </div>
          </div>
          ...
          <div className="bg-[var(--panel-bg)] rounded-xl p-4 border border-yellow-500/10 space-y-2">
            ...
            <div className="text-sm text-[var(--text-secondary)]">
              <span className="text-yellow-600 dark:text-yellow-400 font-bold">{combat.armorPercent.toFixed(0)}% Armor</span> from {playerStats.analysis} Analysis reduces boss damage.
              {combat.armorPercent >= 30 && <span className="text-emerald-700 dark:text-emerald-400"> Excellent damage mitigation.</span>}
              {combat.armorPercent < 30 && combat.armorPercent >= 15 && <span className="text-yellow-600 dark:text-yellow-400"> Decent protection.</span>}
              {combat.armorPercent < 15 && <span className="text-red-600 dark:text-red-400"> Vulnerable — equip Analysis gear.</span>}
            </div>
          </div>
          ...
          <div className="bg-[var(--panel-bg)] rounded-xl p-4 border border-emerald-500/10 space-y-2">
            ...
            <div className="text-sm text-[var(--text-secondary)]">
              <span className="text-emerald-700 dark:text-emerald-400 font-bold">{combat.maxHp} HP</span> from {playerStats.charisma} Charisma.
              {combat.maxHp >= 150 && <span className="text-emerald-700 dark:text-emerald-400"> Large health pool — can absorb many hits.</span>}
              {combat.maxHp < 150 && combat.maxHp >= 120 && <span className="text-yellow-600 dark:text-yellow-400"> Average survivability.</span>}
              {combat.maxHp < 120 && <span className="text-red-600 dark:text-red-400"> Fragile — Charisma gear increases HP.</span>}
            </div>
          </div>
        </div>
          ...
          <div className="space-y-1.5">
            {gearScore < 100 && (
              <div className="flex items-start gap-2 text-[11px] text-[var(--text-tertiary)]">
                ...
                <span>Your Gear Score is <span className="text-amber-600 dark:text-amber-400 font-bold">{gearScore}</span>. Complete more missions and level up to earn better equipment drops.</span>
              </div>
            )}
            {playerStats.tech <= 15 && (
              <div className="flex items-start gap-2 text-[11px] text-[var(--text-tertiary)]">
                <ArrowUpRight className="w-3 h-3 text-blue-600 dark:text-blue-400 mt-0.5 shrink-0" />
                <span>Equip items with <span className="text-blue-600 dark:text-blue-400 font-bold">Tech</span> stats to increase boss damage output.</span>
              </div>
            )}
            {combat.critChance < 0.1 && (
              <div className="flex items-start gap-2 text-[11px] text-[var(--text-tertiary)]">
                <ArrowUpRight className="w-3 h-3 text-green-600 dark:text-green-400 mt-0.5 shrink-0" />
                <span>Socket <span className="text-green-600 dark:text-green-400 font-bold">Focus</span> gems to boost critical hit chance beyond {(combat.critChance * 100).toFixed(0)}%.</span>
              </div>
            )}
            {combat.armorPercent < 10 && (
              <div className="flex items-start gap-2 text-[11px] text-[var(--text-tertiary)]">
                <ArrowUpRight className="w-3 h-3 text-yellow-600 dark:text-yellow-400 mt-0.5 shrink-0" />
                <span>Your armor is low. <span className="text-yellow-600 dark:text-yellow-400 font-bold">Analysis</span> items will reduce boss damage significantly.</span>
              </div>
            )}
