        ...
        <div className="space-y-2">
          ...
          {items.map((item, idx) => (
            <div key={idx} className="flex gap-2">
              ...
              {items.length > 1 && <button type="button" aria-label={`Delete objective ${idx + 1}`} onClick={() => onUpdate({ ...block, items: items.filter((_, i) => i !== idx) })} className="p-1 text-red-600 dark:text-red-400"><Trash2 className="w-3.5 h-3.5" /></button>}
            </div>
          ))}
        ...
        <div className="space-y-2">
          ...
          {options.map((opt, idx) => (
            <div key={idx} className="flex items-center gap-2">
              ...
              {options.length > 2 && <button type="button" aria-label={`Delete option ${String.fromCharCode(65 + idx)}`} onClick={() => { const n = options.filter((_, i) => i !== idx); onUpdate({ ...block, options: n, correctAnswer: block.correctAnswer === idx ? 0 : (block.correctAnswer || 0) > idx ? (block.correctAnswer || 0) - 1 : block.correctAnswer }); }} className="p-1 text-red-600 dark:text-red-400"><Trash2 className="w-3.5 h-3.5" /></button>}
            </div>
          ))}
        ...
        <div className="space-y-2">
          ...
          {answers.map((ans, idx) => (
            <div key={idx} className="flex gap-2">
              ...
              {answers.length > 1 && <button type="button" aria-label={`Delete accepted answer ${idx + 1}`} onClick={() => onUpdate({ ...block, acceptedAnswers: answers.filter((_, i) => i !== idx) })} className="p-1 text-red-600 dark:text-red-400"><Trash2 className="w-3.5 h-3.5" /></button>}
            </div>
          ))}
        ...
        <div className="space-y-2">
          {terms.map((t, idx) => (
            <div key={idx} className="flex gap-2">
              ...
              {terms.length > 1 && <button type="button" aria-label={`Delete vocabulary term ${idx + 1}`} onClick={() => onUpdate({ ...block, terms: terms.filter((_, i) => i !== idx) })} className="p-1 text-red-600 dark:text-red-400"><Trash2 className="w-3.5 h-3.5" /></button>}
            </div>
          ))}
        ...
        <div className="space-y-2">
          ...
          {items.map((item, idx) => (
            <div key={idx} className="flex gap-2"><div className="w-5 h-5 rounded border-2 border-[var(--text-muted)] shrink-0 mt-1.5" />
              ...
              {items.length > 1 && <button type="button" aria-label={`Delete checklist item ${idx + 1}`} onClick={() => onUpdate({ ...block, items: items.filter((_, i) => i !== idx) })} className="p-1 text-red-600 dark:text-red-400"><Trash2 className="w-3.5 h-3.5" /></button>}
            </div>
          ))}
        ...
        <div className="space-y-2">
          ...
          {sortItems.map((item, idx) => (
            <div key={idx} className="flex gap-2">
              ...
              {sortItems.length > 1 && <button type="button" aria-label={`Delete sort item ${idx + 1}`} onClick={() => onUpdate({ ...block, sortItems: sortItems.filter((_, i) => i !== idx) })} className="p-1 text-red-600 dark:text-red-400"><Trash2 className="w-3.5 h-3.5" /></button>}
            </div>
          ))}
        ...
        <div className="space-y-2">
          ...
          {columns.map((col, idx) => (
            <div key={idx} className="flex gap-2">
              ...
              {columns.length > 1 && <button type="button" aria-label={`Delete column ${idx + 1}`} onClick={() => onUpdate({ ...block, columns: columns.filter((_, i) => i !== idx) })} className="p-1 text-red-600 dark:text-red-400"><Trash2 className="w-3.5 h-3.5" /></button>}
            </div>
          ))}
        ...
        <div className="space-y-2">
          ...
          {items.map((item, idx) => (
            <div key={idx} className="flex gap-2">
              ...
              {items.length > 1 && <button type="button" aria-label={`Delete rank item ${idx + 1}`} onClick={() => onUpdate({ ...block, items: items.filter((_, i) => i !== idx) })} className="p-1 text-red-600 dark:text-red-400"><Trash2 className="w-3.5 h-3.5" /></button>}
            </div>
          ))}
          <button type="button" onClick={() => onUpdate({ ...block, items: [...items, ''] })} className="text-xs text-[var(--accent-text)] flex items-center gap-1"><Plus className="w-3 h-3" /> Add Item</button>
        </div>
