...
import { useLocale, useT, useInterpolate, LOCALES, LOCALE_LABELS } from '../lib/i18n';
...
const JoinClassSection: React.FC<{ userId: string }> = ({ userId }) => {
  ...
  const handleRedeem = async () => {
    if (code.replace('-', '').length < 4) { setError(t('settings.enrollment.errorCode')); return; }
    setIsRedeeming(true);
    setError(null);
    try {
      const result = await dataService.redeemEnrollmentCode(code, userId);
      if (result.success) {
        setSuccess(interpolate('settings.enrollment.success', { className: result.classType ?? '' }));
        toast.success(interpolate('settings.enrollment.joinedToast', { className: result.classType ?? '' }));
        setCode('');
      } else {
        setError(result.error || t('settings.enrollment.errorRedeem'));
      }
    } catch {
      setError(t('settings.enrollment.errorGeneric'));
    } finally {
  ...
  return (
    <div className="mb-6">
      <label className="block text-[11.5px] font-bold text-[var(--text-muted)] uppercase tracking-widest mb-2 px-1">{t('settings.section.enrollment')}</label>
      <div className="p-4 bg-[var(--surface-glass)] rounded-2xl border border-[var(--border)]">
        <label className="flex items-center gap-2 text-xs font-bold text-[var(--text-primary)] mb-2">
          ...
          {t('settings.enrollment.joinTitle')}
        </label>
        <div className="flex gap-2">
          ...
          >
            {isRedeeming ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : t('settings.enrollment.join')}
          </button>
        </div>
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={t('settings.title')} maxWidth="max-w-md">
      <div className="space-y-3">
        <div className="mb-4">
          <label className="block text-[11.5px] font-bold text-[var(--text-muted)] uppercase tracking-widest mb-2 px-1">{t('settings.section.visuals')}</label>
        ...
        <div className="mb-4">
          <label className="block text-[11.5px] font-bold text-[var(--text-muted)] uppercase tracking-widest mb-2 px-1">{t('settings.section.privacy')}</label>
          <div className="space-y-2">
            <SettingRow
              icon={Shield}
            ...
            {localSettings.privacyMode && (
              <div className="p-4 bg-[var(--surface-glass)] rounded-2xl border border-[var(--border)]">
                <label className="text-xs font-bold text-[var(--text-secondary)] block mb-2">{t('settings.codename.label')}</label>
                <input
                  type="text"
                  value={codename}
                  onChange={(e) => setCodename(e.target.value.slice(0, 24))}
                  placeholder={t('settings.codename.placeholder')}
                  ...
                  maxLength={24}
                />
                <p className="text-[11.5px] text-[var(--text-tertiary)] mt-1">{interpolate('settings.codename.chars', { count: codename.length })}</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="mb-6">
          <label className="block text-[11.5px] font-bold text-[var(--text-muted)] uppercase tracking-widest mb-2 px-1">{t('settings.section.interface')}</label>
          <div className="space-y-2">
            {/* ─── Language (same segmented pattern as Appearance) ─── */}
            <div className="p-4 bg-[var(--surface-glass)] rounded-2xl border border-[var(--border)]">
              <label className="text-xs font-bold text-[var(--text-secondary)] block mb-2">{t('settings.language.label')}</label>
              <div className="flex gap-2 p-1.5 bg-[var(--surface-sunken)] border border-[var(--border)] rounded-xl">
                {LOCALES.map(loc => (
                  <button
                    key={loc}
              ...
              <p className="text-xs text-[var(--text-tertiary)] leading-tight mt-2">{t('settings.language.description')}</p>
            </div>
            <SettingRow
        ...
        <div className="mb-6">
          <label className="block text-[11.5px] font-bold text-[var(--text-muted)] uppercase tracking-widest mb-2 px-1">{t('settings.section.notifications')}</label>
          <div className="space-y-2">
            {isPushSupported() ? (
              <div className="flex items-center justify-between p-4 bg-[var(--surface-glass)] rounded-2xl border border-[var(--border)] hover:border-[var(--border-accent)] transition">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h4 className="font-bold text-[var(--text-primary)] text-sm">{t('settings.push.title')}</h4>
                  ...
                  onClick={async () => {
                    if (localSettings.pushNotifications) {
                      ...
                      if (perm === 'granted') {
                        ...
                        toast.error(t('settings.push.deniedToast'));
                      }
                    }
                  }}
        ...
        {/* ─── Appearance ─── */}
        <div className="mb-6">
          <label className="block text-[11.5px] font-bold text-[var(--text-muted)] uppercase tracking-widest mb-2 px-1">{t('settings.section.appearance')}</label>
