            ...
            <div className="space-y-4 text-[var(--text-primary)]">
                ...
                <div className="flex gap-4">
                    ...
                    <div className="w-40 shrink-0">
                        <div className="bg-gradient-to-b from-purple-900/20 to-[var(--panel-bg)] rounded-2xl border border-[var(--border)] p-3 flex flex-col items-center">
                            ...
                            <div className="text-center mt-2">
                                ...
                                <div className="text-[11.5px] font-bold mt-0.5" style={{ color: rank.tierColor }}>{rank.rankName}</div>
                                <div className="text-[11.5px] font-black text-[var(--accent-text)] mt-1">Level {level}</div>
                    ...
                    <div className="flex-1 space-y-3">
                        ...
                        <div className="flex gap-3">
                            <div className="flex-1 bg-[var(--panel-bg)] rounded-xl border border-[var(--border)] p-3">
                                <div className="flex items-center justify-between">
                                    <div>
                                        <div className="text-[11.5px] text-[var(--text-muted)] uppercase font-bold tracking-wider">Cyber-Flux</div>
                        ...
                        <div className="bg-[var(--panel-bg)] rounded-xl border border-[var(--border)] p-3">
                            <div className="text-[11.5px] text-[var(--text-muted)] uppercase font-bold tracking-wider mb-2">Combat Stats</div>
                            <div className="grid grid-cols-2 gap-x-4 gap-y-2">
                                {STAT_KEYS.map(key => (
                                    <div key={key} className="flex items-center gap-2">
                                        <span className={`text-[11.5px] font-bold uppercase w-16 ${STAT_COLORS[key].text}`}>{key}</span>
                                        ...
                                        <span className="text-[11.5px] font-mono text-white/80 w-8 text-right">{stats[key]}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                    ...
                    <div className="flex gap-1.5 bg-[var(--panel-bg)] p-1.5 rounded-xl border border-[var(--border)]">
                        {classTabs.map(cls => (
                            <button key={cls} onClick={() => { setActiveClassTab(cls); setSelectedItem(null); }}
                                className={`px-3 py-1.5 rounded-lg text-[11.5px] font-bold uppercase tracking-wide transition cursor-pointer ${selectedClass === cls ? 'bg-purple-600 text-white' : 'text-[var(--text-tertiary)] hover:text-[var(--text-primary)] hover:bg-[var(--surface-glass)]'}`}>
                ...
                <div>
                    <div className="flex items-center justify-between mb-2">
                        <h4 className="text-[11.5px] font-bold text-[var(--text-muted)] uppercase tracking-widest">
                            Active Loadout {selectedClass !== '_legacy' && <span className="text-[var(--accent-text)] ml-1">({selectedClass})</span>}
                        </h4>
                        <span className="text-[11.5px] text-[var(--text-muted)] font-mono">{Object.values(profile.equipped).filter(Boolean).length}/8 slots</span>
                    </div>
...
const StoredGearSection: React.FC<StoredGearSectionProps> = ({
    ...
    return (
        <div>
                ...
                <div className="flex gap-1.5">
                    {customItems.length > 0 && (
                        <button
                            ...
                            className={`px-2.5 py-1 rounded-lg text-[11.5px] font-bold uppercase tracking-wide transition flex items-center gap-1 cursor-pointer ${
            ...
            {inventory.length === 0 && (
                <div className="text-center py-6 text-[var(--text-muted)] text-[11.5px] italic">Inventory Empty</div>
            )}
        ...
        <div className={`rounded-xl border-2 ${colors.border} ${colors.bg} p-4 space-y-3`}>
            <div className="flex items-start justify-between">
                <div className="flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <div className="text-[11.5px] text-[var(--text-muted)]">{item.rarity} {item.slot} &middot; {item.baseName}</div>
                        {item.description && <div className="text-[11.5px] text-[var(--text-tertiary)] italic mt-1">{item.description}</div>}
                    </div>
                </div>
            ...
            {Object.keys(item.stats).length > 0 && (
                <div className="flex gap-3">
                    {Object.entries(item.stats).filter(([, v]) => v).map(([key, val]) => (
                        <div key={key} className={`text-[11.5px] font-bold ${STAT_COLORS[key]?.text || 'text-[var(--text-secondary)]'}`}>
                            +{val} {key}
                        </div>
                    ))}
                </div>
            ...
            {(item.sockets ?? 0) > 0 && (
                <div className="flex items-center gap-2">
                    <div className="text-[11.5px] text-[var(--text-muted)] uppercase font-bold tracking-wider">Sockets</div>
    ...
    return (
        <div className={`rounded-xl border-2 ${colors.border} bg-[var(--panel-bg)] p-4 space-y-3`}>
            <div className="flex items-center justify-between">
                ...
                <div className="flex gap-1.5">
                    <button onClick={onCancel} className="px-3 py-1 text-[11.5px] text-[var(--text-tertiary)] hover:text-[var(--text-primary)] transition cursor-pointer">Cancel</button>
                    <button onClick={onSave} className="px-3 py-1 bg-emerald-600 text-white text-[11.5px] font-bold rounded-lg hover:bg-emerald-500 transition flex items-center gap-1 cursor-pointer"><Save className="w-3 h-3" /> Save</button>
                </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2">
                <div className="col-span-2">
                    <label className="block text-[11.5px] text-[var(--text-muted)] uppercase font-bold mb-0.5">Item Name</label>
                ...
                <div>
                    <label className="block text-[11.5px] text-[var(--text-muted)] uppercase font-bold mb-0.5">Base Name</label>
            ...
            <div className="grid grid-cols-4 gap-2">
                <div>
                    <label className="block text-[11.5px] text-[var(--text-muted)] uppercase font-bold mb-0.5">Rarity</label>
                ...
                <div>
                    <label className="block text-[11.5px] text-[var(--text-muted)] uppercase font-bold mb-0.5">Slot</label>
                ...
                <div>
                    <label className="block text-[11.5px] text-[var(--text-muted)] uppercase font-bold mb-0.5">Visual</label>
                ...
                <div>
                    <label className="block text-[11.5px] text-[var(--text-muted)] uppercase font-bold mb-0.5">Sockets</label>
            ...
            {/* Stats */}
            <div>
                <label className="block text-[11.5px] text-[var(--text-muted)] uppercase font-bold mb-1">Stats</label>
                <div className="grid grid-cols-4 gap-2">
                    {STAT_KEYS.map(key => (
                        <div key={key}>
            ...
            {/* Description */}
            <div>
                <label className="block text-[11.5px] text-[var(--text-muted)] uppercase font-bold mb-0.5">Description</label>
            ...
            <div>
                ...
                {item.affixes.map((a, i) => (
                    <div key={i} className="flex items-center gap-1.5 mb-1">
                        <select value={a.type} onChange={e => updateAffix(i, { type: e.target.value as 'PREFIX' | 'SUFFIX' | 'UNIQUE' })} className="bg-[var(--panel-bg)] border border-[var(--border)] rounded px-1.5 py-1 text-[11.5px] text-[var(--text-primary)] cursor-pointer">
                            <option value="PREFIX">PREFIX</option>
                            <option value="SUFFIX">SUFFIX</option>
                            <option value="UNIQUE">UNIQUE</option>
                        </select>
                        <input value={a.name} onChange={e => updateAffix(i, { name: e.target.value })} placeholder="Name" className="flex-1 bg-[var(--panel-bg)] border border-[var(--border)] rounded px-1.5 py-1 text-[11.5px] text-[var(--text-primary)] focus:outline-none" />
                        <select value={a.stat} onChange={e => updateAffix(i, { stat: e.target.value })} className="bg-[var(--panel-bg)] border border-[var(--border)] rounded px-1.5 py-1 text-[11.5px] text-[var(--text-primary)] cursor-pointer">
            ...
            <div>
                ...
                {(item.effects || []).map((eff, i) => (
                    <div key={i} className="flex items-center gap-1.5 mb-1">
                        <input value={eff.name} onChange={e => updateEffect(i, { name: e.target.value })} placeholder="Effect name" className="w-32 bg-[var(--panel-bg)] border border-[var(--border)] rounded px-1.5 py-1 text-[11.5px] text-[var(--text-primary)] focus:outline-none" />
                        <input value={eff.description} onChange={e => updateEffect(i, { description: e.target.value })} placeholder="Description" className="flex-1 bg-[var(--panel-bg)] border border-[var(--border)] rounded px-1.5 py-1 text-[11.5px] text-[var(--text-primary)] focus:outline-none" />
                        <select value={eff.type} onChange={e => updateEffect(i, { type: e.target.value as 'XP_BOOST' | 'STAT_BOOST' | 'SPECIAL' })} className="bg-[var(--panel-bg)] border border-[var(--border)] rounded px-1.5 py-1 text-[11.5px] text-[var(--text-primary)] cursor-pointer">
    ...
    return (
        <div className="rounded-xl border border-amber-500/20 bg-amber-900/10 p-3 space-y-2">
            ...
            <div className="grid grid-cols-4 gap-2 max-h-48 overflow-y-auto custom-scrollbar">
                {filtered.map(item => {
                    ...
                    return (
                        <div key={item.id} className={`rounded-lg border ${colors.border} ${colors.bg} p-2 flex flex-col gap-1 items-center`}>
                            <ItemIcon visualId={item.visualId} slot={item.slot} rarity={item.rarity} size="w-8 h-8" />
                            <div className={`text-[11.5px] font-bold ${colors.text} truncate w-full text-center`}>{item.name}</div>
                ...
                {filtered.length === 0 && (
                    <div className="col-span-4 text-center py-4 text-[var(--text-muted)] text-[11.5px] italic">
