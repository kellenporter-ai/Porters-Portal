    ...
    return (
        <div className="space-y-4">
            ...
            {deployments.map((deployment, i) => {
                ...
                return (
                    <div key={i} className="bg-[var(--panel-bg)] border border-[var(--border)] rounded-2xl p-5 flex flex-col md:flex-row justify-between items-center gap-6 group hover:border-purple-500/30 transition-all">
                        <div className="flex items-center gap-4 w-full md:w-auto">
                            <div className="relative">
                                <img src={deployment.user.avatarUrl} className="w-12 h-12 rounded-xl border border-[var(--border)] shadow-lg" alt={deployment.user.name} loading="lazy" />
                                <div className="absolute -bottom-1 -right-1 w-4 h-4 bg-green-500 border-2 border-black rounded-full shadow-lg"></div>
                            </div>
                            <div className="min-w-0">
                                <div className="font-bold text-[var(--text-primary)] text-base truncate">{deployment.user.name}</div>
                            ...
                            <div className="text-center md:text-right">
                                ...
                                <div className={`text-xs font-black px-2 py-0.5 rounded border ${
                                    deployment.status === 'DEPLOYED' 
                                        ...
                                        : 'text-[var(--text-secondary)] bg-[var(--surface-glass)] border-[var(--border)]'
                                }`}>
                                    {deployment.status}
                                </div>
                            </div>
                            ...
                            {deployment.status === 'DEPLOYED' && (
                                <>
                                    <div className="text-center px-4 border-l border-[var(--border)]">
                                        <div className="text-[9px] font-bold text-[var(--text-muted)] uppercase tracking-widest mb-1">
