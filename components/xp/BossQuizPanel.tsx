...
const BossQuizPanel: React.FC<BossQuizPanelProps> = ({ classType }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      {quizzes.map(quiz => {
        const hpPercent = (quiz.currentHp / quiz.maxHp) * 100;
        ...
        return (
          <div key={quiz.id} className="rounded-2xl border border-amber-500/20 bg-gradient-to-br from-amber-950/30 to-black/50 p-5 space-y-4">
            {/* Boss info */}
            <div>
            ...
            <div>
              <div className="flex justify-between text-xs mb-1">
                <span className="text-red-400 font-mono">{quiz.currentHp} HP</span>
                ...
                <div className="space-y-2">
                  {question.options.map((option, idx) => {
                    ...
                    return (
                      <button
                        key={idx}
                        onClick={() => handleAnswer(quiz.id, question.id, idx)}
