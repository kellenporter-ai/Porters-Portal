...
const FluxShopPanel: React.FC<FluxShopPanelProps> = ({
  ...
  const handlePurchase = async (item: FluxShopItem) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast.success(isOwnedColor
          ? interpolate('shop.toastEquippedName', { name: item.name })
          : interpolate('shop.toastPurchased', { name: item.name }));
      }
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : t('shop.purchaseFailed');
      sfx.error();
      toast.error(message.includes('Insufficient') ? t('shop.toastNotEnoughFlux') :
                  message.includes('limit') ? t('shop.toastDailyLimit') : message);
    } finally {
      setPurchasing(null);
    }
  };
  ...
  const handleEquipCosmetic = async (cosmeticId: string) => {
    ...
    try {
      ...
      toast.success(newValue ? t('shop.toastEquipCosmetic') : t('shop.toastUnequipCosmetic'));
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : t('shop.equipFailed');
      toast.error(message.includes('do not own') ? t('shop.toastNoOwnCosmetic') : message);
    } finally {
      setEquipping(null);
    }
  };
  ...
  const getTimeRemaining = (expiresAt: string) => {
    ...
    return hrs > 0
      ? interpolate('shop.timeLeft.hours', { hrs, mins: mins % 60 })
      : interpolate('shop.timeLeft.minutes', { mins });
  };
  ...
  const getCategoryLabel = (type: string) => {
    switch (type) {
      case 'XP_BOOST': return t('shop.catXPBoosts');
      case 'REROLL_TOKEN': return t('shop.catUtility');
      case 'NAME_COLOR': return t('shop.catNameColors');
      default: return t('shop.catItems');
    }
  };

  const getCosmeticSubLabel = (visualType: CosmeticVisualType) => {
    switch (visualType) {
      case 'AURA': return t('shop.subAuras');
      case 'PARTICLE': return t('shop.subParticles');
      case 'FRAME': return t('shop.subFrames');
      case 'TRAIL': return t('shop.subTrails');
    }
  };
    ...
    <div className="space-y-4">
      ...
      <div className="bg-gradient-to-br from-cyan-500/10 to-purple-500/10 border border-[var(--border)] rounded-2xl p-3 backdrop-blur-sm">
        <div className="flex items-center flex-wrap gap-x-3 gap-y-1.5">
          <h2 className="text-base font-bold text-[var(--text-primary)] tracking-tight">{t('shop.title')}</h2>
          <p className="text-xs text-[var(--text-tertiary)]">{t('shop.subtitle')}</p>
          ...
          {currentBoosts.map((boost, i) => (
            <div key={i} className="flex items-center gap-1 bg-yellow-500/10 border border-yellow-500/20 px-2 py-0.5 rounded-md">
              ...
              <span className="text-[11.5px] font-bold text-yellow-300">{interpolate('shop.xpBoostActive', { pct: Math.round((boost.value - 1) * 100), time: getTimeRemaining(boost.expiresAt) })}</span>
            </div>
          ))}
          {rerollTokens > 0 && (
            <div className="flex items-center gap-1 bg-blue-500/10 border border-blue-500/20 px-2 py-0.5 rounded-md">
              ...
              <span className="text-[11.5px] font-bold text-blue-300">{interpolate('shop.rerolls', { count: rerollTokens, s: rerollTokens !== 1 ? 's' : '' })}</span>
            </div>
          )}
          {nameColor && (
            <div className="flex items-center gap-1 bg-[var(--surface-glass)] border border-[var(--border)] px-2 py-0.5 rounded-md">
              ...
              <span className="text-[11.5px] font-bold text-[var(--text-secondary)]">{t('shop.nameColorStatus')}</span>
            </div>
          )}
        </div>
      </div>
        ...
        <div className="flex-1 min-w-0 space-y-4">
          ...
          <div className="space-y-2">
              ...
              {searchQuery && (
                <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-2">
                  <span className="text-[11.5px] text-[var(--text-muted)] font-mono">
                    {interpolate('shop.results', { count: totalFilteredCount })}
                  </span>
                  <button onClick={() => setSearchQuery('')} className="text-[var(--text-muted)] hover:text-[var(--text-primary)] transition" aria-label={t('shop.clearSearch')}>
            ...
            <div className="flex gap-1.5 overflow-x-auto pb-1" style={{ scrollbarWidth: 'none' }}>
              {([
                { key: 'all', label: t('shop.tabAll'), icon: <ShoppingCart className="w-3.5 h-3.5" /> },
                { key: 'boosts', label: t('shop.tabBoosts'), icon: <Sparkles className="w-3.5 h-3.5" /> },
                { key: 'utility', label: t('shop.tabUtility'), icon: <RotateCcw className="w-3.5 h-3.5" /> },
                { key: 'cosmetics', label: t('shop.tabCosmetics'), icon: <Palette className="w-3.5 h-3.5" /> },
                ...(ENABLE_3D_AVATAR ? [{ key: 'models', label: t('shop.tabModels'), icon: <CuboidIcon className="w-3.5 h-3.5" /> }] : []),
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            {item.dailyLimit > 0 && (
                              <p className="text-xs text-[var(--text-tertiary)] mt-1 font-mono">
                                {interpolate('shop.dailyCount', { count: dailyCount, limit: item.dailyLimit })}
                              </p>
                            )}
                          </div>
                          {isNameColor && isOwnedColor ? (
                            isActiveColor ? (
                              ...
                              <button
                                ...
                                aria-label={interpolate('shop.ariaEquipName', { name: item.name })}
                              ...
                              >
                                {isPurchasing ? (
                                  <span className="animate-pulse">{t('shop.equipping')}</span>
                                ) : (
                                  <>
                            ...
                            <button
                              ...
                              disabled={disabled}
                              aria-label={atLimit
                                ? interpolate('shop.ariaLimitReached', { name: item.name })
                                : isNameColor && item.cost === 0
                                  ? interpolate('shop.ariaSelectName', { name: item.name })
                                  : interpolate('shop.ariaPurchaseFor', { name: item.name, cost: item.cost })}
                            ...
                            >
                              {atLimit ? (
                                ...
                                <span className="animate-pulse">{t('shop.processing')}</span>
                              ) : isNameColor && item.cost === 0 ? (
                                <>
                                  <Palette className="w-4 h-4" aria-hidden="true" />
                                  {t('shop.select')}
                                ...
                                <>
                                  ...
                                  {item.cost} {t('shop.currencyLabel')}
                                </>
                              )}
                            </button>
                          )}
          ...
          {(shopTab === 'all' || shopTab === 'models') && ENABLE_3D_AVATAR && filteredCharacterModelItems.length > 0 && (
            <div className="space-y-3">
              <div className="flex items-center gap-2 px-1">
                ...
                <h3 className="text-sm font-bold text-[var(--text-secondary)] uppercase tracking-widest">{t('shop.modelsTitle')}</h3>
              </div>
              <p className="text-xs text-[var(--text-muted)] px-1">{t('shop.modelsSubtitle')}</p>
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        {isOwned ? (
                          <button
                            ...
                            disabled={isSelected}
                            aria-label={isSelected ? interpolate('shop.ariaSelectedName', { name: model.name }) : interpolate('shop.ariaSelectName', { name: model.name })}
                          ...
                          <button
                            ...
                            aria-label={interpolate('shop.ariaPurchaseFor', { name: model.name, cost: model.cost })}
                          ...
                          >
                            {isPurchasing ? (
                              ...
                              <>
                                <Hexagon className="w-4 h-4" aria-hidden="true" />
                                {model.cost} {t('shop.currencyLabel')}
                              </>
                            )}
                          </button>
                        ) : null}
            ...
            {(shopTab === 'all' || shopTab === 'cosmetics') && (
              <div className="space-y-2">
                <div className="flex items-center gap-2 px-1">
                  ...
                  <h3 className="text-sm font-bold text-[var(--text-secondary)] uppercase tracking-widest">{t('shop.cosmeticsTitle')}</h3>
                </div>
                <p className="text-xs text-[var(--text-muted)] px-1">{t('shop.cosmeticsSubtitle')}</p>
              </div>
            )}
              ...
              return (
                <div key={visualType} className="space-y-3">
                  <div className="flex items-center gap-2 px-1 ml-2">
                    ...
                    <span className="text-[11.5px] text-[var(--text-muted)] font-mono ml-auto">{interpolate('shop.priceEach', { price: subColors.price })}</span>
                  </div>
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-start gap-3">
                              ...
                              {playerEquipped && (
                                <button
                                  ...
                                  aria-label={previewCosmeticId === item.id ? interpolate('shop.ariaStopPreview', { name: item.name }) : interpolate('shop.ariaPreviewName', { name: item.name })}
                            ...
                            <button
                              ...
                              aria-label={isEquipped ? interpolate('shop.ariaUnequipName', { name: item.name }) : interpolate('shop.ariaEquipName', { name: item.name })}
                      ...
                      return (
                        ...
                        >
                          ...
                          <button
                            ...
                            onClick={() => handlePurchase(item)}
                            disabled={disabled}
                            aria-label={interpolate('shop.ariaPurchaseFor', { name: item.name, cost: item.cost })}
            ...
            <div className="lg:w-[42%] flex-shrink-0 lg:sticky lg:top-4 lg:self-start">
              <div className="space-y-4">
                ...
                <div className="flex items-center gap-2 bg-[var(--panel-bg)] px-3 py-1.5 rounded-xl border border-cyan-500/20 justify-center">
                  ...
                  <span className="text-xs text-[var(--text-muted)] font-bold">{t('shop.currencyLabel')}</span>
                </div>
                ...
                <div className="bg-[var(--panel-bg)] border border-[var(--border)] rounded-2xl p-4 backdrop-blur-sm">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h3 className="text-sm font-bold text-[var(--text-secondary)] uppercase tracking-widest">{t('shop.agentPreview')}</h3>
                    {previewCosmeticId && (
                      <button
                        type="button"
                        ...
                        aria-label={t('shop.ariaClearPreview')}
                      >
                  ...
                  <div className="mt-3 flex items-center gap-3">
                    ...
                    <span className="text-base font-bold" style={nameColor ? { color: nameColor } : undefined}>
                      {codename || t('shop.defaultCodename')}
                    </span>
                  </div>
                  ...
                  <div className="mt-3 flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      {previewCosmeticDef ? (
                        ...
                        <div>
                          <span className="text-sm font-bold text-[var(--text-secondary)]">{t('shop.equippedLabel')}</span>
                          <div className="flex flex-wrap gap-1 mt-1">
                            ...
                            {!activeCosmetics?.aura && !activeCosmetics?.particle && !activeCosmetics?.frame && !activeCosmetics?.trail && (
                              <p className="text-xs text-[var(--text-muted)]">{t('shop.previewHint')}</p>
                            )}
                          </div>
                        </div>
                      )}
