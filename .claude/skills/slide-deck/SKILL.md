...
- **Image placeholders.** Since we can't embed real images, include clear placeholder boxes with descriptive labels like `[Diagram: Free body diagram of a block on an incline]` styled as visible placeholder cards. Suggest the user run `/generate-image` to create matching visuals.
